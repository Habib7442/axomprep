...
const ChapterCard = ({
  ...
  return (
    <article 
      className="chapter-card rounded-lg p-4 shadow-md hover:shadow-lg transition-shadow"
      style={{ borderLeft: `4px solid ${getSubjectColor(subject)}` }}
    >
      <div className="flex justify-between items-start">
        <div>
          <h3 className="font-bold text-lg">{title}</h3>
          <p className="text-sm text-gray-600">{subject} • Class {chapterClass}</p>
        </div>
      ...
      <Link href={`/chapters/${id}`} className="mt-4 block">
